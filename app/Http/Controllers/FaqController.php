...
class FaqController extends Controller
{
    public function customer_faq(Request $request)
    {
        ...
        if($input['lang'] == 'en'){
            $data = Faq::select('id','question','answer','status')->where('status',1)->where('user_type_id',1)->where('country_id',$input['country_id'])->get();
        }else{
            $data = Faq::select('id','question_ar as question','answer_ar as answer','status')->where('status',1)->where('user_type_id',1)->where('country_id',$input['country_id'])->get();
        }
