...
class NotificationController extends Controller
{
    public function get_customer_notification_messages(Request $request)
    {
        ...
        if($input['lang'] == 'en'){
            $data = NotificationMessage::select('id','title','message','image','status')->where('status',1)->where('type',1)->where('country_id',$input['country_id'])->orderBy('id', 'DESC')->get();
        }else{
            $data = NotificationMessage::select('id','title_ar as title','message_ar as message','image','status')->where('status',1)->where('type',1)->where('country_id',$input['country_id'])->orderBy('id', 'DESC')->get();
        }
