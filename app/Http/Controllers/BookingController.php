...
class BookingController extends Controller
{
    public function ride_confirm(Request $request)
    {
        ...
        $drivers = $database->getReference($input['country_id'].'/drivers/'.$input['vehicle_type'].'/'.$input['zone'])
        ...
        if(!$drivers){
            $this->add_missed_trip_request($input['customer_id'],$input['pickup_lat'],$input['pickup_lng'],$input['country_id'],$input['zone']);
        ...
        if($input['trip_type'] != 5){
            foreach($drivers as $key => $value){
                if($value && array_key_exists('gender', $value)){
                    ...
                    //echo "<pre>";
                    //print_r($distance.'-'.$driver_wallet);
        ...
        if($input['trip_type'] == 3){
            $fares = $this->calculate_outstation_fare($input['vehicle_type'],$input['km'],$input['promo'],$input['country_id'],1,$input['trip_sub_type'],$input['surge']);
        }
        
        if($input['trip_type'] == 4){
            $fares = $this->calculate_delivery_fare($input['vehicle_type'],$input['km'],$input['promo'],$input['country_id'],1,$input['trip_sub_type'],$input['surge']);
        }
        ...
        if($input['trip_type'] == 5 && $shared_ride_mode == 2){
            ...
            $newPost = $database
            ->getReference($input['country_id'].'/drivers/'.$input['vehicle_type'].'/'.$input['zone'].'/'.$min_driver_id.'/booking')
            ->update([
                'booking_id' => $id,
                'booking_status' => 1,
                'pickup_address' => $input['pickup_address'],
                ...
                'trip_type_ar'=>DB::table('trip_types')->where('id',$input['trip_type'])->value('name_ar')
            ]);
        }
    ...
    public function find_driver($trip_request_id){
        ...
        $drivers = $database->getReference($trip_request->country_id.'/drivers/'.$trip_request->vehicle_type.'/'.$trip_request->zone)
                    ->getSnapshot()->getValue();
        ...
        if($min_driver_id == 0){
            $this->add_missed_trip_request($trip_request->customer_id,$trip_request->pickup_lat,$trip_request->pickup_lng,$trip_request->country_id,$trip_request->zone);
            $newPost = $database
        ...
        if($trip_request->trip_type == 5 && $shared_ride_mode == 2){
            ...
            $newPost = $database
            ->getReference($trip_request->country_id.'/drivers/'.$trip_request->vehicle_type.'/'.$trip_request->zone.'/'.$min_driver_id.'/booking')
            ->update([
                'booking_id' => $trip_request->id,
                'booking_status' => 1,
                'pickup_address' => $trip_request->pickup_address,
    ...
    public function trip_reject(Request $request)
    {
        ...
        if($input['from'] == 1){
            $newPost = $database
            ->getReference($trip->country_id.'/drivers/'.$trip->vehicle_type.'/'.$trip->zone.'/'.$input['driver_id'].'/booking')
            ->update([
                'booking_id' => 0,
                'booking_status' => 0,
                'pickup_address' => "",
    ...
    public function trip_accept(Request $request)
    {
        ...
        if($trip_details->trip_type == 5 && $shared_ride_count > 0){
           ...
           $newPost = $database
            ->getReference($trip_details->country_id.'/drivers/'.$trip['vehicle_type'].'/'.$trip_details->zone.'/'.$input['driver_id'].'/booking')
            ->update([
                'booking_id' => $id,
                'booking_status' => 2
            ]); 
    ...
    public function trip_cancel_by_customer(Request $request)
    {
        ...
        $newPost = $database
        ->getReference($trip->country_id.'/drivers/'.$vehicle_type.'/'.$trip->zone.'/'.$trip->driver_id.'/booking')
        ->update([
            'booking_id' => 0,
            'booking_status' => 0,
            'customer_name' => '',
    ...
    public function get_recent_places(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
            'customer_id' => 'required',
        ...
        if($input['type'] == 1){
            $data = Trip::select('id','pickup_address as address','pickup_lat as lat','pickup_lng as lng')->where('customer_id',$input['customer_id'])->orderBy('id', 'DESC')->limit(2)->get();
        }else{
            $data = Trip::select('id','drop_address as address','drop_lat as lat','drop_lng as lng')->where('customer_id',$input['customer_id'])->orderBy('id', 'DESC')->limit(2)->get();
        }
    ...
    public function change_statuses(Request $request)
    {
        ...
        ->update([
            ...
            'customer_status_name_ar' => $current_status->customer_status_name_ar,
            'status' => $current_status->id,
            'driver_status_name' => $current_status->status_name,
            'driver_status_name_ar' => $current_status->status_name_ar,
            'new_status' => $new_status->id,
            'new_driver_status_name' => $new_status->status_name,
            'new_driver_status_name_ar' => $new_status->status_name_ar
        ]);
    ...
    public function calculate_fare($trip_id){
        ...
        if($distance != 0 && is_object($trip)){
            if($trip->trip_type == 1){
                $fare = $this->calculate_daily_fare($trip->vehicle_type,$distance,$trip->promo_code,$trip->country_id,$trip->surge);
               ...
               $fare = $this->calculate_rental_fare($trip->vehicle_type,$trip->package_id,$trip->promo_code,$trip->country_id,$distance,$hours,$trip->surge);
                ...
                $fare = $this->calculate_outstation_fare($trip->vehicle_type,$distance,$trip->promo_code,$trip->country_id,$days,$trip->trip_sub_type,$trip->surge);
                ...
                $fare = $this->calculate_delivery_fare($trip->vehicle_type,$distance,$trip->promo_code,$trip->country_id,$days,$trip->trip_sub_type,$trip->surge);
            
            }else if($trip->trip_type == 5){
                $fare = $this->calculate_shared_fare($trip->vehicle_type,$distance,$trip->promo_code,$trip->country_id,$trip->surge);
            }
    ...
    public function update_payment_mode($trip_id,$trip,$fare){
        ...
        if($is_subscription_trip){
            Trip::where('id',$trip_id)->update(['payment_method' => PaymentMethod::where('country_id',$trip->country_id)->where('payment_type',4)->value('id') ]);
        ...
        if($customer_wallet && $customer_wallet > 0){
            ...
            if($remaining_fare >= 0){
                ...
                $payment_history['amount'] = $fare;
                PaymentHistory::create($payment_history);
                CustomerWalletHistory::create(['country_id' => $trip->country_id, 'customer_id' => $trip->customer_id, 'type' => 2, 'message' => 'Amount debited for booking(#'.$trip->trip_id.')','message_ar' => 'ام لم للحجز(#'.$trip->trip_id.')', 'amount' => $fare, 'transaction_type' => 3 ]);
                return 0;
            }else{
                ...
                CustomerWalletHistory::create(['country_id' => $trip->country_id, 'customer_id' => $trip->customer_id, 'type' => 2, 'message' => 'Amount debited for booking(#'.$trip->trip_id.')','message_ar' => 'المبلغ المخو حز(#'.$trip->trip_id.')', 'amount' => $customer_wallet, 'transaction_type' => 3 ]);
    ...
    public function direct_booking(Request $request){
        ...
        $newPost = $database
        ->getReference($data['country_id'].'/drivers/'.$data['vehicle_type'].'/'.$data['zone'].'/'.$input['driver_id'].'/booking')
        ->update([
            'booking_id' => $id,
            'booking_status' => 1,
    ...
    public function auto_trip_accept($trip_id,$driver_id)
    {
        ...
        $newPost = $database
        ->getReference($trip['country'].'/drivers/'.$trip['vehicle_type'].'/'.$trip['zone'].'/'.$input['driver_id'].'/booking')
        ->update([
            'booking_id' => $id,
            'booking_status' => 2
        ]);
   ...
   public function get_drop_addresses(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
            'trip_id' => 'required',
        ...
        $result = DB::table('stops')->where('trip_id',$input['trip_id'])->get();
    ...
    public function get_cancellation_policy(Request $request){
        ...
        $data = DB::table('privacy_policies')->where('country_id',$input['country_id'])->where('slug','cancellation_policy')->first();
    ...
    public function trip_request_cancel_by_customer(Request $request)
    {
        ...
        $newPost = $database
        ->getReference($trip->country_id.'/drivers/'.$trip->vehicle_type.'/'.$trip->zone.'/'.$trip->driver_id.'/booking')
        ->update([
            'booking_id' => 0,
            'booking_status' => 0,
            'customer_name' => '',
    ...
    public function get_promo_codes(Request $request){
        ...
        if($input['lang'] == 'en'){
            $data = [];
            $codes = PromoCode::select('id','promo_name','promo_code','description','promo_type','discount','redemptions','min_fare','max_discount_value','status')->where('country_id',$input['country_id'])->where('customer_id',$input['customer_id'])->orWhere('country_id',$input['country_id'])->where('customer_id',0)->get();
            ...
            $data = [];
            $codes = PromoCode::select('id','promo_name_ar as promo_name','promo_code_ar as promo_code','description_ar as description','promo_type','discount','redemptions','min_fare','max_discount_value','status')->where('country_id',$input['country_id'])->where('customer_id',$input['customer_id'])->orWhere('country_id',$input['country_id'])->where('customer_id',0)->get();
            foreach($codes as $key => $value){
                $using_count = CustomerPromoHistory::where('customer_id',$input['customer_id'])->where('promo_id',$value->id)->where('status',1)->count();
                if($value->redemptions > $using_count){
                    array_push($data,$value);
    ...
    public function update_shared_current_booking($trip_id){
        ...
        $newPost = $database
            ...
            ->update([
                ...
                "static_map" => $trip->static_map,
                'trip_type'=> $trip_type->name,
                'trip_type_ar'=>$trip_type->name_ar
            ]);
    }
    ...
    public function get_heatmap_coordinates(Request $request){
        ...
        $data = MissedTripRequest::where('created_at','>=',$formatted_date)->where('country_id',$input['country_id'])->where('zone',$input['zone'])->select('latitude','longitude','weight')->get();
    ...
    public function check_surge(Request $request){
        ...
        $data = DB::table('missed_trip_requests')->where('created_at','>=',$formatted_date)->where('country_id',$input['country_id'])->where('zone',$input['zone'])->select('latitude','longitude','weight')->get();
    ...
    public function add_missed_trip_request($customer_id,$latitude,$longitude,$country_id,$zone){
    ...
    public function get_tips(Request $request){
        ...
        $result['data'] = DB::table('tips')->where('country_id',$input['country_id'])->pluck('tip');
