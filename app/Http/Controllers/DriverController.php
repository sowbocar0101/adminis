...
class DriverController extends Controller
{
    ...
    public function vehicle_type_list(Request $request)
    {
        ...
        if($input['lang'] == 'en'){
            $data = VehicleCategory::select('id','vehicle_type','description','base_fare','price_per_km','active_icon','inactive_icon','status')->where('country_id',$input['country_id'])->get();
        }else{
            $data = VehicleCategory::select('id','vehicle_type_ar as vehicle_type','description_ar as description','base_fare','price_per_km','active_icon','inactive_icon','status')->where('country_id',$input['country_id'])->get();
        }
    ...
    public function vehicle_update(Request $request)
    {
        ...
        $zone = Driver::where('id',$input['driver_id'])->value('zone');
        ...
        if (is_object($driver_vehicles)) {
            ...
            $newPost = $database
            ->getReference($input['country_id'].'/'.'drivers/'.$input['vehicle_type'].'/'.$zone.'/'.$input['driver_id'])
            ->update([
                'driver_id' => $input['driver_id'],
                'vehicle_type' => $input['vehicle_type'],
                'vehicle_mode' => $vehicle_mode,
                'driver_name' => Driver::where('id',$input['driver_id'])->value('first_name'),
                'gender' => Driver::where('id',$input['driver_id'])->value('gender'),
                'online_status' => 0,
                'geo' => $geo,
                'booking' => $booking
            ]);
    ...
    public function register(Request $request)
    {
        ...
        if (is_object($driver)) {
            if($refered_by != '' && $referrel_id){
                ...
                $refered_country_id = Driver::where('id',$referrel_id)->value('country_id');
                DriverWalletHistory::create([ 'country_id' => $refered_country_id, 'driver_id' => $referrel_id, 'type' => 3, 'message' => $content, 'amount' => $referral_amount]);
            }
            $driver->referral_code = 'CAB'.str_pad($driver->id,5,"0",STR_PAD_LEFT);
    ...
    public function login(Request $request){
        ...
        if (Hash::check($credentials['password'], $driver->password)) {
            if($driver->status == 1){
                ...
                }else if(!is_object($vehicle_status)){
                    return response()->json([
                        "result" => $driver,
    ...
    public function driver_wallet(Request $request){
        ...
        if($input['lang'] == 'en'){
            ...
            $data['wallets'] = DriverWalletHistory::select('id','type','message','amount','created_at')->where('driver_id',$input['id'])->get();
        }else{
            $data['wallet_amount'] = Driver::where('id',$input['id'])->value('wallet');
        
            $data['wallets'] = DriverWalletHistory::select('id','type','message_ar as message','amount','created_at')->where('driver_id',$input['id'])->get();
        }
    ...
    public function change_online_status(Request $request){
        ...
        if($driver->id_proof_status && is_object($vehicle_status) && $vehicle_document_status){
            ...
            $database = $factory->createDatabase();
            $newPost = $database
            ->getReference($driver->country_id.'/drivers/'.$vehicle_status->vehicle_type.'/'.$driver->zone.'/'.$driver->id)
            ->update([
    ...
    public function get_tutorials(Request $request)
    {
        ...
        if($input['lang'] == 'en'){
            $data = DriverTutorial::select('id','title','description','file','thumbnail_image','status')->where('country_id',$input['country_id'])->orderBy('id', 'DESC')->get();
        }else{
            $data = DriverTutorial::select('id','title_ar as title','description_ar as description','file','thumbnail_image','status')->where('country_id',$input['country_id'])->orderBy('id', 'DESC')->get();
        }
    ...
    public function update_document(Request $request){
        ...
        DB::table($input['table'])->where($input['find_field'],'=',$input['find_value'])->update([
            $input['update_field'] => $input['update_value'], $input['status_field'] => 15
        ]);
        return response()->json([
            "message" => 'Success',
            "status" => 1
    ...
    public function change_driver_settings(Request $request){
        ...
        $validator = Validator::make($input, [
            ...
            'driver_hiring_status' => 'required',
        ]);
        if ($validator->fails()) {
            return $this->sendError($validator->errors());
        }
        $data = DB::table('drivers')->where('id','=',$input['driver_id'])->update([
            'shared_ride_status' => $input['shared_ride_status'] , 'driver_hiring_status' => $input['driver_hiring_status']  
        ]);
        return response()->json([
            "data" =>$data,
            "message" => 'Success',
