...
use App\Status;
use App\Country;
use App\Models\DeliveryFareManagement;
...
class DeliveryFareManagementController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Delivery Fare Management';
    ...
    {
        $grid = new Grid(new DeliveryFareManagement());
    ...
    {
        $show = new Show(DeliveryFareManagement::findOrFail($id));
    ...
    {
        $form = new Form(new DeliveryFareManagement());

        $statuses = Status::where('type','general')->pluck('name','id');
        $vehiclecategories = VehicleCategory::pluck('vehicle_type', 'id');
        $trip_sub_type = TripSubType::where('trip_type','4')->pluck('trip_sub_type', 'id');
        $countries = Country::pluck('country_name', 'id');

        $form->select('country_id', __('Country'))->load('vehicle_type', '/admin/get_delivery_vehicle_type', 'id', 'vehicle_type')->options($countries)->rules(function ($form) {
            return 'required';
            });
